...
const bookMultipleAppointment = async (req, res) => {
  try {
    ...
    // Paystack payment initialization - FIXED
    const paystackResponse = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        ...
        amount: Math.round(calculatedAmount * 100), // Convert to kobo
        currency: 'NGN', // CHANGED TO NGN
        reference: `apt_${newAppointment._id}_${Date.now()}`,
        callback_url: `${process.env.FRONTEND_URL}/appointment/verify/${newAppointment._id}`,
        metadata: {
          ...
          custom_fields: [
            {
              ...
              value: services.length > 1 ? 'Multiple Services' : services[0].serviceTitle
            },
            {
    ...
    console.log("💳 Payment URL:", paystackData.data.authorization_url);
...
const verifyAppointmentPayment = async (req, res) => {
  try {
    ...
    const { reference, appointmentId } = req.body; // Changed from sessionId to reference
    ...
    if (!reference || !appointmentId) {
      ...
      return res.status(400).json({ 
        success: false,
        message: "Payment reference and Appointment ID are required" 
      });
    }
    ...
    try {
      ...
      console.log("📊 Transaction gateway response:", transaction.gateway_response);

      if (transaction.status === 'abandoned' || transaction.status === 'failed') {
        console.log("❌ Payment failed or abandoned");
        return res.status(200).json({
          success: false,
          message: `Payment ${transaction.gateway_response || transaction.status}. Please try again.`,
