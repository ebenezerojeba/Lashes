// FRONTEND - Paystack verification
...
export default function VerifyAppointment() {
  ...
  // Paystack returns 'reference' and 'trxref' in the callback URL
  ...
  const isValidReference = (ref) => typeof ref === 'string' && ref.trim() !== '';
  ...
  const verifyPayment = async (attempts = 0) => {
    ...
    try {
      const { data, status: httpStatus } = await axios.post(
        ...
        { reference, appointmentId }, // Changed from sessionId to reference
        config
      );
      ...
      if (data.status === 'processing' || (data.payment_status === 'pending' && attempts < 8)) {
  ...
  useEffect(() => {
    console.log('🏁 Starting Paystack verification process');
    ...
    if (!appointmentId || !reference) {
      setStatus('error');
      setMessage('Missing required parameters (appointment ID or payment reference)');
      return;
    }

    if (!isValidId(appointmentId) || !isValidReference(reference)) {
      setStatus('error');
